...
function collides(t, e) {
  let i = getWorldRect(t),
    s = getWorldRect(e);
  return (
    !((t.radius && i.width != i.height) || (e.radius && s.width != s.height)) &&
    (([i, s] = [i, s].map((s) => ((s == i ? t : e).radius && ((s.radius = s.width / 2), (s.x += s.radius), (s.y += s.radius)), s))),
    t.radius && e.radius
      ? Math.hypot(i.x - s.x, i.y - s.y) < i.radius + s.radius
      : t.radius || e.radius
        ? circleRectCollision(t.radius ? i : s, t.radius ? e : t)
        : i.x < s.x + s.width && i.x + i.width > s.x && i.y < s.y + s.height && i.y + i.height > s.y)
  );
}
function getWorldRect(t) {
  let {x: e = 0, y: i = 0, width: s, height: a, radius: n} = t.world || t;
  return (
...
let noop = () => {},
  srOnlyStyle = 'position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0);',
  focusParams = {preventScroll: !0};
...
function circleRectCollision(t, e) {
  let {x: i, y: s, width: a, height: n} = getWorldRect(e);
  do {
    ((i -= e.sx || 0), (s -= e.sy || 0));
  } while ((e = e.parent));
  let r = t.x - Math.max(i, Math.min(t.x, i + a)),
    o = t.y - Math.max(s, Math.min(t.y, s + n));
  return r * r + o * o < t.radius * t.radius;
}
let canvasEl,
  context,
  callbacks$2 = {};
...
function init$1(t, {contextless: e = !1} = {}) {
  ...
  return ((context = canvasEl.getContext('2d') || new Proxy({}, handler$1)), (context.imageSmoothingEnabled = !1), emit('init'), {canvas: canvasEl, context: context});
}
class Animation {
  constructor({spriteSheet: t, frames: e, frameRate: i, loop: s = !0, name: a}) {
    let {width: n, height: r, spacing: o = 0, margin: h = 0} = t.frame;
    Object.assign(this, {spriteSheet: t, frames: e, frameRate: i, loop: s, name: a, width: n, height: r, spacing: o, margin: h, isStopped: !1, _f: 0, _a: 0});
  }
  clone() {
  ...
  update(t = 1 / 60) {
    if (!this.isStopped)
      if (this.loop || this._f != this.frames.length - 1) for (this._a += t; this._a * this.frameRate >= 1; ) ((this._f = ++this._f % this.frames.length), (this._a -= 1 / this.frameRate));
      else this.stop();
  }
  render({x: t, y: e, width: i = this.width, height: s = this.height, context: a = getContext()}) {
    let n = (this.frames[this._f] / this.spriteSheet._f) | 0,
      r = this.frames[this._f] % this.spriteSheet._f | 0;
    a.drawImage(this.spriteSheet.image, this.margin + r * this.width + (2 * r + 1) * this.spacing, this.margin + n * this.height + (2 * n + 1) * this.spacing, this.width, this.height, t, e, i, s);
  }
}
function factory$b() {
...
function joinPath(t, e) {
  return [t.replace(trailingSlash, ''), t ? e.replace(leadingSlash, '') : e].filter((t) => t).join('/');
}
...
function loadImage(t) {
  return (
    ...
    new Promise((e, i) => {
      let s, a, n;
      if (((s = joinPath(imagePath, t)), imageAssets[s])) return e(imageAssets[s]);
      ((a = new Image()),
        (a.onload = function () {
...
function loadAudio(t) {
  return new Promise((e, i) => {
    ...
    return (
      (s = new Audio()),
      (a = getCanPlay(s)),
      (t = [].concat(t).reduce((t, e) => t || (a[getExtension(e)] ? e : null), 0))
        ? ((n = joinPath(audioPath, t)),
          audioAssets[n]
            ? e(audioAssets[n])
            : (s.addEventListener('canplay', function () {
                ((r = getUrl(n, window.location.href)), (audioAssets[getName(t)] = audioAssets[n] = audioAssets[r] = this), emit('assetLoaded', this, t), e(this));
              }),
              (s.onerror = function () {
  ...
  return (
    ...
    dataAssets[e]
      ...
      : fetch(e)
          ...
          .then((s) => ((i = getUrl(e, window.location.href)), 'object' == typeof s && dataMap.set(s, i), (dataAssets[getName(t)] = dataAssets[e] = dataAssets[i] = s), emit('assetLoaded', s, t), s))
  );
}
function load(...t) {
  return (
    ...
    Promise.all(
      t.map((t) => {
        ...
        return e.match(imageRegex) ? loadImage(t) : e.match(audioRegex) ? loadAudio(t) : loadData(t);
      })
    )
  );
}
...
class GameObject extends Updatable {
  init({
    ...
    ((this._c = []),
      super.init({width: t, height: e, context: i, anchor: r, opacity: o, rotation: h, drotation: d, ddrotation: l, scaleX: c, scaleY: u, ...p}),
      (this._di = !0),
      this._uw(),
      this.addChild(n),
      (this._rf = s),
  ...
  render() {
    ...
    let e = this.width,
      i = this.height;
    this.radius && (e = i = 2 * this.radius);
    let s = -e * this.anchor.x,
      a = -i * this.anchor.y;
    ((s || a) && t.translate(s, a), (this.context.globalAlpha = this.opacity), this._rf(), (s || a) && t.translate(-s, -a), this.children.map((t) => t.render && t.render()), t.restore());
  }
  draw() {}
  _pc() {
  ...
  _uw() {
    if (!this._di) return;
    let {_wx: t = 0, _wy: e = 0, _wo: i = 1, _wrot: s = 0, _wsx: a = 1, _wsy: n = 1} = this.parent || {};
    ((this._wx = this.x),
      (this._wy = this.y),
      (this._ww = this.width),
      (this._wh = this.height),
      this.radius && ((this._wrx = this.radius), (this._wry = this.radius)),
      ...
      this.radius && ((this._wrx = this.radius * this._wsx), (this._wry = this.radius * this._wsy)),
      (this._wrot = s + this.rotation));
  ...
  addChild(...t) {
    t.flat().map((t) => {
      (this.children.push(t), (t.parent = this), (t._pc = t._pc || noop), t._pc());
    });
  }
  removeChild(...t) {
    t.flat().map((t) => {
  ...
  advance(t) {
    (super.advance(t), (this.drotation += this.ddrotation), (this.rotation += this.drotation));
  }
  setScale(t, e = t) {
...
class Sprite extends GameObject {
  ...
  draw() {
    if (
      (this.image && this.context.drawImage(this.image, 0, 0, this.image.width, this.image.height),
      this.currentAnimation && this.currentAnimation.render({x: 0, y: 0, width: this.width, height: this.height, context: this.context}),
      this.color)
    ) {
      if (((this.context.fillStyle = this.color), this.radius)) return (this.context.beginPath(), this.context.arc(this.radius, this.radius, this.radius, 0, 2 * Math.PI), void this.context.fill());
...
class Text extends GameObject {
  ...
  _p() {
    ...
    if (
      ...
      this._fw &&
        e.map((e) => {
          let i = e.split(' '),
            ...
            a = s;
          (i.map((e) => {
            ((a += ' ' + e), t.measureText(a).width > this._fw && (this._s.push(s), (a = e)), (s = a));
          }),
            this._s.push(a));
        }),
      ...
      let i = 0;
      (e.map((e) => {
        (this._s.push(e), (i = Math.max(i, t.measureText(e).width)));
      }),
        (this._w = this._fw || i));
    }
  ...
  draw() {
    ...
    ((e = this.textAlign || ('rtl' == i.canvas.dir ? 'right' : 'left')),
      (t = 'right' == e ? this.width : 'center' == e ? (this.width / 2) | 0 : 0),
      this._s.map((s, a) => {
        ((i.textBaseline = 'top'),
          (i.textAlign = e),
          (i.fillStyle = this.color),
          (i.font = this.font),
          this.strokeColor && ((i.strokeStyle = this.strokeColor), (i.lineWidth = this.lineWidth ?? 1), i.strokeText(s, t, this._fs * this.lineHeight * a)),
...
function getCurrentObject(t) {
  ...
  for (let i = e.length - 1; i >= 0; i--) {
    let s = e[i];
    if (s.collidesWithPointer ? s.collidesWithPointer(t) : circleRectCollision(t, s)) return s;
  }
}
...
function getCanvasOffset(t) {
  let {canvas: e, _s: i} = t,
    ...
    a = 'none' != i.transform ? i.transform.replace('matrix(', '').split(',') : [1, 1, 1, 1],
    n = parseFloat(a[0]),
    r = parseFloat(a[3]),
    o = (getPropValue(i, 'border-left-width') + getPropValue(i, 'border-right-width')) * n,
    h = (getPropValue(i, 'border-top-width') + getPropValue(i, 'border-bottom-width')) * r,
    d = (getPropValue(i, 'padding-left') + getPropValue(i, 'padding-right')) * n,
    l = (getPropValue(i, 'padding-top') + getPropValue(i, 'padding-bottom')) * r;
  return {
    scaleX: (s.width - o - d) / e.width,
    scaleY: (s.height - h - l) / e.height,
    offsetX: s.left + (getPropValue(i, 'border-left-width') + getPropValue(i, 'padding-left')) * n,
    offsetY: s.top + (getPropValue(i, 'border-top-width') + getPropValue(i, 'padding-top')) * r,
  };
}
...
function callCallback(t, e, i) {
  ...
  (s && s[e] && s[e](i), callbacks$1[e] && callbacks$1[e](i, s), 'onOver' == e && (s != t._oo && t._oo && t._oo.onOut && t._oo.onOut(i), (t._oo = s)));
}
function pointerHandler(t, e) {
  ...
  t.type.includes('touch')
    ? (Array.from(t.touches).map(({clientX: t, clientY: e, identifier: i}) => {
        let h = s.touches[i];
        (h || ((h = s.touches[i] = {start: {x: (t - r) / a, y: (e - o) / n}}), s.touches.length++), (h.changed = !1));
      }),
      Array.from(t.changedTouches).map(({clientX: i, clientY: h, identifier: d}) => {
        let l = s.touches[d];
        ((l.changed = !0),
          (l.x = s.x = (i - r) / a),
          (l.y = s.y = (h - o) / n),
          ...
          'onUp' == e && (delete s.touches[d], s.touches.length--, s.touches.length || emit('touchEnd')));
      }))
    : ((s.x = (t.clientX - r) / a), (s.y = (t.clientY - o) / n), callCallback(s, e, t));
}
...
function track(...t) {
  t.flat().map((t) => {
    let e = t.context ? t.context.canvas : getCanvas(),
      i = pointers.get(e);
    if (!i) throw new ReferenceError('Pointer events not initialized for the objects canvas');
    t.__r ||
      ((t.__r = t.render),
      (t.render = function () {
...
function untrack(...t) {
  t.flat().map((t) => {
    let e = t.context ? t.context.canvas : getCanvas(),
      i = pointers.get(e);
    if (!i) throw new ReferenceError('Pointer events not initialized for the objects canvas');
    ((t.render = t.__r), (t.__r = 0), removeFromArray(i._o, t));
  });
}
...
class Button extends Sprite {
  init({padX: t = 0, padY: e = 0, text: i, disabled: s = !1, container: a, onDown: n, onUp: r, ...o} = {}) {
    (super.init({padX: t, padY: e, ...o}),
      (this.textNode = factory$7({...i, context: this.context})),
      this.width || ((this.width = this.textNode.width), (this.height = this.textNode.height)),
      track(this),
      this.addChild(this.textNode),
      (this._od = n || noop),
      (this._ou = r || noop));
    let h = (this._dn = document.createElement('button'));
    ((h.style = srOnlyStyle),
      (h.textContent = this.text),
      s && this.disable(),
      h.addEventListener('focus', () => this.focus()),
      h.addEventListener('blur', () => this.blur()),
      h.addEventListener('keydown', (t) => this._kd(t)),
      h.addEventListener('keyup', (t) => this._ku(t)),
      addToDom(h, a ?? this.context.canvas),
      this._uw(),
      this._p());
  }
  get text() {
  ...
  focus() {
    this.disabled || ((this.focused = !0), document.activeElement != this._dn && this._dn.focus(focusParams), this.onFocus());
  }
  blur() {
...
function GameLoop({fps: t = 60, clearCanvas: e = !0, update: i = noop, render: s, context: a = getContext(), blur: n = !1} = {}) {
  ...
  let r,
    ...
    f = !0;
  function m() {
    if (((o = requestAnimationFrame(m)), f && ((h = performance.now()), (d = h - r), (r = h), !(d > 1e3)))) {
      for (c += d; c >= u; ) (emit('tick'), l.update(p), (c -= u));
      (g(l.context), l.render());
    }
  }
  return (
    n ||
    ...
    (l = {
      ...
      context: a,
      start() {
        this.isStopped && ((r = performance.now()), (this.isStopped = !1), requestAnimationFrame(m));
      },
      stop() {
...
function updateGamepad() {
  let t = navigator.getGamepads ? navigator.getGamepads() : navigator.webkitGetGamepads ? navigator.webkitGetGamepads : [];
...
function gamepadPressed(t, {gamepad: e} = {}) {
  return isNaN(e) ? gamepads.some((e) => e.pressedButtons[t]) : !!gamepads[e] && !!gamepads[e].pressedButtons[t];
}
    ...
    swipe: {
      ...
      touchend({0: t}) {
        let e = t.x - t.start.x,
          ...
          s = Math.abs(e),
          a = Math.abs(i);
        if (!(s < this.threshold && a < this.threshold)) return s > a ? (e < 0 ? 'left' : 'right') : i < 0 ? 'up' : 'down';
      },
    },
    pinch: {
      touches: 2,
...
let handler = {set: (t, e, i) => (e.startsWith('_') || (t._d = !0), Reflect.set(t, e, i))},
  alignment = {start: (t) => (t ? 1 : 0), center: () => 0.5, end: (t) => (t ? 0 : 1)};
class Grid extends GameObject {
  init({flow: t = 'column', align: e = 'start', justify: i = 'start', colGap: s = 0, rowGap: a = 0, numCols: n = 1, dir: r = '', breakpoints: o = [], ...h} = {}) {
    return (super.init({flow: t, align: e, justify: i, colGap: s, rowGap: a, numCols: n, dir: r, breakpoints: o, ...h}), this._p(), new Proxy(this, handler));
  }
  addChild(t) {
  ...
  _p() {
    ...
    let t = (this._g = []),
      ...
      i = (this._rh = []),
      s = this.children,
      a = (this._nc = 'column' == this.flow ? 1 : 'row' == this.flow ? s.length : this.numCols),
      n = 0,
      r = 0;
    ...
    this._uw();
    let l = ('rtl' == this.context.canvas.dir && !this.dir) || 'rtl' == this.dir;
    ((this._rtl = l), l && ((this._g = t.map((t) => t.reverse())), (this._cw = e.reverse()), (h = h.reverse())));
      ...
      (t.map((t, n) => {
        if (t && !u.includes(t)) {
          ...
          let r = alignment[t.justifySelf || p[n % p.length]](this._rtl),
            ...
            d = t.colSpan || 1,
            l = e[n];
          if (d > 1 && n + d <= this._nc) for (let t = 1; t < d; t++) l += e[n + t] + h[(n + t) % h.length];
          let f = l * r,
            m = i[s] * o,
            _ = 0,
            y = 0,
            {width: w, height: x} = t.world || t;
          if ((t.anchor && ((_ = t.anchor.x), (y = t.anchor.y)), 0 == r)) f += w * _;
          else if (0.5 == r) {
...
let seed,
  ...
  keyupCallbacks = {},
  pressedKeys = {},
  keyMap = {Enter: 'enter', Escape: 'esc', Space: 'space', ArrowLeft: 'arrowleft', ArrowUp: 'arrowup', ArrowRight: 'arrowright', ArrowDown: 'arrowdown'};
...
function initKeys() {
  let t;
  for (t = 0; t < 26; t++) keyMap['Key' + String.fromCharCode(t + 65)] = String.fromCharCode(t + 97);
...
function keyPressed(t) {
  return !![].concat(t).some((t) => pressedKeys[t]);
}
...
function onInput(t, e, {gamepad: i, key: s} = {}) {
  [].concat(t).map((t) => {
    if (contains(t, gamepadMap)) onGamepad(t, e, i);
      ...
      if (!['down', 'up'].includes(t)) throw new TypeError(`"${t}" is not a valid input name`);
      onPointer(t, e);
    }
  });
}
function offInput(t, {gamepad: e, key: i} = {}) {
  [].concat(t).map((t) => {
    contains(t, gamepadMap) ? offGamepad(t, e) : isGesture(t) ? offGesture(t) : contains(t, keyMap) ? offKey(t, i) : ['down', 'up'].includes(t) && offPointer(t);
  });
}
...
function unregisterPlugin(t, e) {
  ...
  i &&
    ...
    Object.getOwnPropertyNames(e).map((t) => {
      let s = getMethod(t);
      t.startsWith('before') ? removeFromArray(i._inc[s].before, e[t]) : t.startsWith('after') && removeFromArray(i._inc[s].after, e[t]);
    });
}
...
function getIndices(t, e) {
  let i = [],
    ...
    n = t.y < a,
    r = t.y + t.height >= a;
  return (t.x < s && (n && i.push(0), r && i.push(2)), t.x + t.width >= s && (n && i.push(1), r && i.push(3)), i);
}
class Quadtree {
  constructor({maxDepth: t = 3, maxObjects: e = 25, bounds: i} = {}) {
    ((this.maxDepth = t), (this.maxObjects = e));
    let s = getCanvas();
    ((this.bounds = i || {x: 0, y: 0, width: s.width, height: s.height}), (this._b = !1), (this._d = 0), (this._o = []), (this._s = []), (this._p = null));
  }
  clear() {
    (this._s.map((t) => {
      t.clear();
  ...
  add(...t) {
    t.flat().map((t) => {
      this._b ? this._a(t) : (this._o.push(t), this._o.length > this.maxObjects && this._d < this.maxDepth && (this._sp(), this._o.map((t) => this._a(t)), (this._o.length = 0)));
    });
  }
  _a(t) {
    ...
    if (((this._b = !0), !this._s.length))
      for (t = (this.bounds.width / 2) | 0, e = (this.bounds.height / 2) | 0, i = 0; i < 4; i++)
        ((this._s[i] = new Quadtree({
          bounds: {x: this.bounds.x + (i % 2 == 1 ? t : 0), y: this.bounds.y + (i >= 2 ? e : 0), width: t, height: e},
...
function rand() {
  ((seed ??= Date.now()), (seed |= 0), (seed = (seed + 2654435769) | 0));
  let t = seed ^ (seed >>> 16);
  return ((t = Math.imul(t, 569420461)), (t ^= t >>> 15), (t = Math.imul(t, 1935289751)), ((t ^= t >>> 15) >>> 0) / 4294967296);
}
function randInt(t, e, i = rand) {
  return ((i() * (e - t + 1)) | 0) + t;
}
function getSeed() {
  return seed;
}
function seedRand(t = Date.now()) {
  if (((seed = t), 'string' == typeof t)) {
    for (var e = 0, i = 1779033703 ^ t.length; e < t.length; e++) i = ((i = Math.imul(i ^ t.charCodeAt(e), 3432918353)) << 13) | (i >>> 19);
    ((i = Math.imul(i ^ (i >>> 16), 2246822507)), (i = Math.imul(i ^ (i >>> 13), 3266489909)), (seed = (i ^= i >>> 16) >>> 0));
  }
}
...
class Scene {
  constructor({id: t, name: e = t, objects: i = [], context: s = getContext(), cullObjects: a = !0, cullFunction: n = collides, sortFunction: r, ...o}) {
    ((this._o = []), Object.assign(this, {id: t, name: e, context: s, cullObjects: a, cullFunction: n, sortFunction: r, ...o}));
    ...
    ((this.camera = new (class extends GameObject {
      set x(t) {
        ((d.sx = t - this.centerX), (super.x = t));
      }
      get x() {
        return super.x;
      }
      set y(t) {
        ((d.sy = t - this.centerY), (super.y = t));
      }
      get y() {
        return super.y;
      }
    })({context: s, anchor: {x: 0.5, y: 0.5}, render: this._rf.bind(this)})),
      this.add(i),
      (this._i = () => {
        this.context ??= getContext();
        let t = this.context.canvas,
          ...
          s = e / 2,
          a = i / 2;
        (Object.assign(this.camera, {centerX: s, centerY: a, x: s, y: a, width: e, height: i}), h.isConnected || addToDom(h, t));
      }),
  ...
  add(...t) {
    t.flat().map((t) => {
      (this._o.push(t), (t.parent = this), this._dn.append(...getAllNodes(t)));
    });
  }
  remove(...t) {
    t.flat().map((t) => {
      (removeFromArray(this._o, t),
        (t.parent = null),
        getAllNodes(t).map((t) => {
          addToDom(t, this.context.canvas);
        }));
    });
  }
  show() {
    ...
    (t ? t.focus(focusParams) : this._dn.focus(focusParams), this.onShow());
  }
  hide() {
  ...
  _rf() {
    let {_o: t, context: e, _sx: i, _sy: s, camera: a, sortFunction: n, cullObjects: r, cullFunction: o} = this;
    e.translate(i, s);
    let h = t;
    (r && (h = h.filter((t) => o(a, t))), n && h.sort(n), h.map((t) => t.render && t.render()));
  }
  render() {
    if (!this.hidden) {
...
class SpriteSheet {
  constructor({image: t, frameWidth: e, frameHeight: i, spacing: s = 0, margin: a = 0, animations: n} = {}) {
    if (!t) throw Error('You must provide an Image for the SpriteSheet');
    ((this.animations = {}), (this.image = t), (this.frame = {width: e, height: i, spacing: s, margin: a}), (this._f = ((t.width - a) / e) | 0), this.createAnimations(n));
  }
  createAnimations(t) {
    let e, i;
    for (i in t) {
      let {frames: s, frameRate: a, loop: n} = t[i];
      if (((e = []), null == s)) throw Error('Animation ' + i + ' must provide a frames property');
      ([].concat(s).map((t) => {
        e = e.concat(parseFrames(t));
      }),
        (this.animations[i] = factory$b({spriteSheet: this, frames: e, frameRate: a, loop: n, name: i})));
    }
  }
}
function factory$1() {
...
let FLIPPED_HORIZONTALLY = 2147483648,
...
class TileEngine {
    ...
    ((h.width = r),
      ...
      n.map((e) => {
        let {__k: i, location: s} = window,
          ...
          {source: n} = e;
        if (n) {
          if (!i) throw Error('You must use "load" or "loadData" to resolve tileset.source');
          let t = i.d[i.u(n, a)];
          if (!t) throw Error(`You must load the tileset source "${n}" before loading the tileset`);
          Object.keys(t).map((i) => {
            e[i] = t[i];
          });
        }
        let {image: r} = e;
        if ('' + r === r) {
          if (!i) throw Error('You must use "load" or "loadImage" to resolve tileset.image');
          let t = i.i[i.u(r, a)];
          if (!t) throw Error(`You must load the image "${r}" before loading the tileset`);
          e.image = t;
        }
      }),
      Object.assign(this, {context: getContext(), layerMap: {}, layerCanvases: {}, mapwidth: r, mapheight: o, _sx: 0, _sy: 0, _o: [], ...t}),
      this.context && this._p(),
      on('init', () => {
        ((this.context ??= getContext()), this._p());
      }));
  }
  get sx() {
    return this._sx;
  ...
  set sx(t) {
    let e = Math.max(0, this.mapwidth - getCanvas().width);
    this._sx = clamp(0, e, t);
  }
  set sy(t) {
    let e = Math.max(0, this.mapheight - getCanvas().height);
    this._sy = clamp(0, e, t);
  }
  set objects(t) {
    (this.remove(this._o), this.add(t));
  }
  ...
  add(...t) {
    t.flat().map((t) => {
      (this._o.push(t), (t.parent = this));
    });
  }
  remove(...t) {
    t.flat().map((t) => {
      (removeFromArray(this._o, t), (t.parent = null));
    });
  }
  getPosition(t) {
    let e = getCanvas().getBoundingClientRect(),
      i = t.x - e.x,
      s = t.y - e.y;
    return ((i += this.sx), (s += this.sy), {x: i, y: s, row: getRow(s, this.tileheight), col: getCol(i, this.tilewidth)});
  }
  ...
  layerCollidesWith(t, e) {
    let {tilewidth: i, tileheight: s, layerMap: a} = this,
      ...
      u = getCol(n + o, i),
      p = a[t];
    for (let t = d; t <= c; t++) for (let e = l; e <= u; e++) if (p.data[e + t * this.width]) return !0;
    return !1;
  }
  tileAtLayer(t, e) {
  ...
  render(t = this._c, e = !0) {
    ...
    (s.drawImage(t, a, n, h, d, 0, 0, h, d), e && (s.save(), (a || n) && s.translate(-a, -n), this.objects.map((t) => t.render && t.render()), s.restore()));
  }
  renderLayer(t) {
    ...
    if (!a) {
      ...
      ((a = document.createElement('canvas')), (n = a.getContext('2d')), (a.width = i), (a.height = r), (e[t] = a), this._rl(s, n));
    }
    (s._d && ((s._d = !1), n.clearRect(0, 0, a.width, a.height), this._rl(s, n)), this.render(a, !1));
  }
  _p() {
    ...
    ((this._d = !1),
      e.map((e) => {
        ...
        ((e._d = !1), (i[s] = e), a && 0 != n && this._rl(e, t));
      }));
  }
  _rl(t, e) {
    ...
    (e.save(),
      ...
      s.map((t, i) => {
        ...
        let s,
          h = 0,
          d = 0,
          l = t & FLIPPED_HORIZONTALLY,
          c = t & FLIPPED_VERTICALLY,
          u = 0,
          p = 0,
          g = 0,
          f = 0,
          m = 0;
        ((h = l || c),
          (m = (t &= ~(FLIPPED_HORIZONTALLY | FLIPPED_VERTICALLY)) & FLIPPED_DIAGONALLY),
          m && (l && c ? (g = 1) : l ? (u = 1) : c ? (p = 1) : (f = 1), (d = u || p || g || f), (t &= ~FLIPPED_DIAGONALLY)));
        for (let e = a.length - 1; e >= 0 && ((s = a[e]), !(t / s.firstgid >= 1)); e--);
        let {image: _, spacing: y = 0, margin: w = 0, firstgid: x, columns: b} = s,
          v = t - x,
          P = b ?? (_.width / (r + y)) | 0,
          C = (i % n) * r,
          k = ((i / n) | 0) * o,
          ...
          S = w + ((v / P) | 0) * (o + y);
        (d
          ? (e.save(), e.translate(C + r / 2, k + o / 2), p || f ? e.rotate(-Math.PI / 2) : (u || g) && e.rotate(Math.PI / 2), (g || f) && e.scale(-1, 1), (C = -r / 2), (k = -o / 2))
          : h && (e.save(), e.translate(C + (l ? r : 0), k + (c ? o : 0)), e.scale(l ? -1 : 1, c ? -1 : 1), (C = h ? 0 : C), (k = h ? 0 : k)),
          e.drawImage(_, A, S, r, o, C, k, r, o),
          (h || d) && e.restore());
      }),
      e.restore());
  }
