...
export function renderTexts(context, canvas, player, currentLevel) {
	...
	if (gameScreen.menu.visible) {
		drawPixel(context, `${gameScreen.menu.options[0].message}`, (canvas.width - `${gameScreen.menu.options[0].message}`.length * 40) / 2, 475, 10, gameScreen.menu.options[0].selected ? '#FEDA94' : undefined);
		drawPixel(context, `${gameScreen.menu.options[1].message}`, (canvas.width - `${gameScreen.menu.options[1].message}`.length * 40) / 2, 575, 10, gameScreen.menu.options[1].selected ? '#FEDA94' : undefined);
	}
